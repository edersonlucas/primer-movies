describe('Movie Page Navigation', () => {
  ...
  context('access and redirection', () => {
    ...
    it('should redirect the user to "/moveis/1" when accessing "/movies/{Negative number or zero}"', () => {
      cy.visit('/movies/0');
      cy.url().should('include', '/movies/1');
    });
  ...
  context('pagination', () => {
    ...
    it('should be possible to navigate between pages by clicking on the next page and previous page button', () => {
      ...
      cy.get('button[aria-label="Botão para a pagina seguinte"]').should('not.be.disabled').click()
      cy.url().should('include', '/movies/2');
      cy.get('button[aria-label="Botão para a pagina seguinte"]').should('be.disabled')
      cy.get('button.pagination-button-selected').should('have.text', '2')
      cy.get('button[aria-label="Botão para pagina anterior"]').should('not.be.disabled').click()
      cy.url().should('include', '/movies/1');
      cy.get('button[aria-label="Botão para a pagina seguinte"]').should('not.be.disabled')
      cy.get('button[aria-label="Botão para pagina anterior"]').should('be.disabled')
      cy.get('button.pagination-button-selected').should('have.text', '1')
    })
